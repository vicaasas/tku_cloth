  ...
  <div>
    ...
    >
      <template slot="table-row" slot-scope="rows">
        <span v-if="rows.column.field === 'class_name'">{{rows.row.class_name}}</span>
        <span v-else-if="rows.column.field === 'student_name'">{{rows.row.student_name}}</span>
        <span v-else-if="rows.column.field === 'student_id'">{{rows.row.student_id}}</span>
        ...
        <span v-else-if="rows.column.field === 'size'">{{rows.row.size}}</span>
        <span v-else-if="rows.column.field === 'color'">{{rows.row.color}}</span>
        <span v-else-if="rows.column.field === 'return'">
          <span v-if="rows.row.return == 0">尚未歸還</span>
          <span v-if="rows.row.return == 1">已歸還</span>
        </span>
        <span v-else-if="rows.column.field === 'has_paid'">
          <span v-if="rows.row.has_paid == 0">尚未繳費</span>
          <span v-if="rows.row.has_paid == 1">已繳費</span>
        </span>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <form v-on:submit.prevent="edit_order">
                  <!--action="edit_order" method="post"-->
        ...
        >
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">您確定歸還衣服</div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">取消</button>
                <button
                  type="button"
        ...
        >
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">您確定要刪除訂單</div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">取消</button>
                <button
                  type="button"
  ...
  methods: {
    onSelectAll: function (data_all) {
      ...
      data_all.selectedRows.forEach((element) => {
        var index = this.select_column_data.findIndex(
          (x) => x.originalIndex == element.originalIndex
        );
        console.log(element);
        if (index == -1) {
        ...
        .then(function (response) {
          ...
          select_column.forEach((element) => {
            var index = rows.findIndex(
              (x) => x.student_id == element.student_id
            );
            rows[index].state = 1;
          });
        })
    ...
    edit_order: function (submitEvent) {
      ...
      axios
        ...
        .then(function (response) {
          var index = rows.findIndex(
            (x) => x.student_id == order_data.student_id.value
          );
    ...
    return {
      columns: [
        ...
        },
        {
          ...
          filterOptions: {
            enabled: true, // enable filter for this column
            placeholder: "選擇繳費狀態", // placeholder for filter input
