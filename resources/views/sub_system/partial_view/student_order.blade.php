  ...
  @if(!$student_order['have_orders']->isEmpty())
    ...
    @foreach($student_order['have_orders'] as $have_orders)
        <tr>
            ...
            @if($student_order->has_paid==0)
            <button id="order_edit" type="button" class="btn btn-primary" data-toggle="modal" data-target="#editModal" data-student_id="{{ $have_orders['student_id'] }}" data-order_id="{{ $student_order['order_id'] }}">編輯</button>

            <button id="order_cancel" type="button" class="btn btn-primary" data-toggle="modal" data-target="#cancelModal" data-student_id="{{ $have_orders['student_id'] }}" data-order_id="{{ $student_order['order_id'] }}">取消</button>
            @else
            <button id="order_edit" type="button" class="btn btn-primary" data-toggle="modal" data-target="#editModal" data-student_id="{{ $have_orders['student_id'] }}" data-order_id="{{ $student_order['order_id'] }}" disabled>編輯</button>

            <button id="order_cancel" type="button" class="btn btn-primary" data-toggle="modal" data-target="#cancelModal" data-student_id="{{ $have_orders['student_id'] }}" data-order_id="{{ $student_order['order_id'] }}" disabled>取消</button>
            @endif
          </th>
        </tr>
    @endforeach
...
$('#cancelModal').on('show.bs.modal', function (event) {
  ...
  var modal = $(this)
  modal.find('.modal-body').text('您確定要取消訂單編號 ' + order_id+' 中 '+student_id+' 的訂單')
  modal.find('#cancel_bt').attr("data-order",order_id);
  modal.find('#cancel_bt').attr("data-student_id",student_id);
});
$('#cancel_bt').click(function() {
  var order_id = $('#cancel_bt').attr('data-order');
  var student_id = $('#cancel_bt').attr('data-student_id');
  console.log(order_id);
  $.ajaxSetup({
      headers: {
