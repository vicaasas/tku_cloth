...
@foreach($student_order as $student_order)
    <tr>
        ...
        @if($student_order->has_get_cloths==0)
          <button id="get_this_cloths" type="button" class="btn btn-primary" data-toggle="modal" data-target="#getclothsModal" data-order_id="{{ $student_order['order_id'] }}">退款</button>
        @else
          <button id="get_this_cloths" type="button" class="btn btn-primary" data-toggle="modal" data-target="#getclothsModal" data-order_id="{{ $student_order['order_id'] }}" disabled>退款</button>
        @endif
        </th>
    </tr>
...
$(document).on('show.bs.modal','#getclothsModal', function (event) {
  ...
  var modal = $(this)
  modal.find('.modal-body').text('要退款訂單編號' + order_id+"?")
  modal.find('#refund').attr("data-order",order_id);
});

$('#refund').click(function() {
  var order_id = $('#refund').attr('data-order');
  ...
  $.ajax({
    type:'POST',
    url:"{{ route('return.determine_refund') }}",
